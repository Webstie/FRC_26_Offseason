...
public class GyroIOPigeon2 implements GyroIO {
  ...
  // Debounce the connection flag like ModuleIOTalonFX does, so a single dropped CAN frame doesn't
  // momentarily flip the gyro to "disconnected" (which forces a kinematics-yaw fallback + alert).
  ...
  public void updateInputs(GyroIOInputs inputs) {
    inputs.connected =
        connectedDebounce.calculate(BaseStatusSignal.refreshAll(yaw, yawVelocity).equals(StatusCode.OK));
